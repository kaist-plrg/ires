...
object `AL::TemplateMiddleList[0,0].EarlyErrors` extends Algo {
  val head = SyntaxDirectedHead("TemplateMiddleList", 0, 0, Rhs(List(NonTerminal("TemplateMiddle", List(""), false), NonTerminal("Expression", List("+In", "?Yield", "?Await"), false)), None), "EarlyErrors", List())
  val ids = List(
