...
object `AL::BindingPropertyList[1,0].BoundNames` extends Algo {
  val head = SyntaxDirectedHead("BindingPropertyList", 1, 0, Rhs(List(NonTerminal("BindingPropertyList", List(""), false), Terminal(","), NonTerminal("BindingProperty", List(""), false)), None), "BoundNames", List())
  val ids = List(
