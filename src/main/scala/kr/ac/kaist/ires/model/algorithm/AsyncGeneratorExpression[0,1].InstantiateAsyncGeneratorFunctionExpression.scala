...
object `AL::AsyncGeneratorExpression[0,1].InstantiateAsyncGeneratorFunctionExpression` extends Algo {
  val head = SyntaxDirectedHead("AsyncGeneratorExpression", 0, 1, Rhs(List(Terminal("async"), Terminal("function"), Terminal("*"), NonTerminal("BindingIdentifier", List(""), false), Terminal("("), NonTerminal("FormalParameters", List(""), false), Terminal(")"), Terminal("{"), NonTerminal("AsyncGeneratorBody", List(""), false), Terminal("}")), None), "InstantiateAsyncGeneratorFunctionExpression", List(Param("name", Optional)))
  val ids = List(
  ...
  |  6:app __x3__ = (OrdinaryFunctionCreate INTRINSIC_AsyncGeneratorFunction_prototype sourceText FormalParameters AsyncGeneratorBody CONST_nonDASHlexicalDASHthis funcEnv)
  |  6:let closure = [! __x3__]
  |  7:app __x4__ = (SetFunctionName closure name)
  |  7:[! __x4__]
  |  8:app __x5__ = (OrdinaryObjectCreate INTRINSIC_AsyncGeneratorFunction_prototype_prototype)
