...
object `AL::CallExpression[5,0].Evaluation` extends Algo {
  val head = SyntaxDirectedHead("CallExpression", 5, 0, Rhs(List(NonTerminal("CallExpression", List(""), false), Terminal("."), NonTerminal("IdentifierName", List(""), false)), None), "Evaluation", List())
  val ids = List(
