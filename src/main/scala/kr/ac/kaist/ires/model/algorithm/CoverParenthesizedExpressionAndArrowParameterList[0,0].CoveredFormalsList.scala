...
object `AL::CoverParenthesizedExpressionAndArrowParameterList[0,0].CoveredFormalsList` extends Algo {
  val head = SyntaxDirectedHead("CoverParenthesizedExpressionAndArrowParameterList", 0, 0, Rhs(List(Terminal("("), NonTerminal("Expression", List(""), false), Terminal(")")), None), "CoveredFormalsList", List())
  val ids = List(
