...
object `AL::CatchFinallyFunctions` extends Algo {
  val head = NormalHead("CatchFinallyFunctions", List())
  val ids = List(
  ...
  |  7:??? "Let id:{thrower} be equivalent to a function that throws id:{reason} ."
  |  8:app __x4__ = (Invoke promise "then" (new [thrower]))
  |  8:return [? __x4__]
  |}""".stripMargin)
