...
object `AL::ForInOfStatement[8,0].EarlyErrors` extends Algo {
  val head = SyntaxDirectedHead("ForInOfStatement", 8, 0, Rhs(List(Terminal("for"), Terminal("await"), Terminal("("), NonTerminal("ForDeclaration", List(""), false), Terminal("of"), NonTerminal("AssignmentExpression", List(""), false), Terminal(")"), NonTerminal("Statement", List(""), false)), None), "EarlyErrors", List())
  val ids = List(
