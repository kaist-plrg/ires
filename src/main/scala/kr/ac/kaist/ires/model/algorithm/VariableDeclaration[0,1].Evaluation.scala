...
object `AL::VariableDeclaration[0,1].Evaluation` extends Algo {
  val head = SyntaxDirectedHead("VariableDeclaration", 0, 1, Rhs(List(NonTerminal("BindingIdentifier", List(""), false), NonTerminal("Initializer", List(""), false)), None), "Evaluation", List())
  val ids = List(
