...
object `AL::AsyncGeneratorExpression[0,1].EarlyErrors` extends Algo {
  val head = SyntaxDirectedHead("AsyncGeneratorExpression", 0, 1, Rhs(List(Terminal("async"), Terminal("function"), Terminal("*"), NonTerminal("BindingIdentifier", List(""), true), Terminal("("), NonTerminal("FormalParameters", List(""), false), Terminal(")"), Terminal("{"), NonTerminal("AsyncGeneratorBody", List(""), false), Terminal("}")), None), "EarlyErrors", List())
  val ids = List(
