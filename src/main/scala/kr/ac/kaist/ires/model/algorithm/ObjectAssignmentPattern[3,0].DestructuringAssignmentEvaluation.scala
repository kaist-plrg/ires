...
object `AL::ObjectAssignmentPattern[3,0].DestructuringAssignmentEvaluation` extends Algo {
  val head = SyntaxDirectedHead("ObjectAssignmentPattern", 3, 0, Rhs(List(Terminal("{"), NonTerminal("AssignmentPropertyList", List(""), false), Terminal(","), Terminal("}")), None), "DestructuringAssignmentEvaluation", List(Param("value", Normal)))
  val ids = List(
