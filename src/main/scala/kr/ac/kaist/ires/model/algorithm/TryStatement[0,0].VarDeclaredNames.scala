...
object `AL::TryStatement[0,0].VarDeclaredNames` extends Algo {
  val head = SyntaxDirectedHead("TryStatement", 0, 0, Rhs(List(Terminal("try"), NonTerminal("Block", List(""), false), NonTerminal("Catch", List(""), false)), None), "VarDeclaredNames", List())
  val ids = List(
