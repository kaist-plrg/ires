...
object `AL::OptionalChain[6,0].ChainEvaluation` extends Algo {
  val head = SyntaxDirectedHead("OptionalChain", 6, 0, Rhs(List(NonTerminal("OptionalChain", List(""), false), Terminal("."), NonTerminal("IdentifierName", List(""), false)), None), "ChainEvaluation", List(Param("baseValue", Normal), Param("baseReference", Normal)))
  val ids = List(
