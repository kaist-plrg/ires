...
object `AL::ContinueStatement[1,0].EarlyErrors` extends Algo {
  val head = SyntaxDirectedHead("ContinueStatement", 1, 0, Rhs(List(Terminal("continue"), NonTerminal("LabelIdentifier", List(""), false), Terminal(";")), None), "EarlyErrors", List())
  val ids = List(
