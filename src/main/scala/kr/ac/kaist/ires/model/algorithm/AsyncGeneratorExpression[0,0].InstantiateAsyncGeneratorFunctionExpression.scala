...
object `AL::AsyncGeneratorExpression[0,0].InstantiateAsyncGeneratorFunctionExpression` extends Algo {
  val head = SyntaxDirectedHead("AsyncGeneratorExpression", 0, 0, Rhs(List(Terminal("async"), Terminal("function"), Terminal("*"), Terminal("("), NonTerminal("FormalParameters", List(""), false), Terminal(")"), Terminal("{"), NonTerminal("AsyncGeneratorBody", List(""), false), Terminal("}")), None), "InstantiateAsyncGeneratorFunctionExpression", List(Param("name", Optional)))
  val ids = List(
  ...
  |  3:app __x1__ = (OrdinaryFunctionCreate INTRINSIC_AsyncGeneratorFunction_prototype sourceText FormalParameters AsyncGeneratorBody CONST_nonDASHlexicalDASHthis scope)
  |  3:let closure = [! __x1__]
  |  4:app __x2__ = (SetFunctionName closure name)
  |  4:__x2__
  |  5:app __x3__ = (OrdinaryObjectCreate INTRINSIC_AsyncGeneratorFunction_prototype_prototype)
