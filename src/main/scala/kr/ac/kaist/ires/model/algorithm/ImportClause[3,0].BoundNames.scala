...
object `AL::ImportClause[3,0].BoundNames` extends Algo {
  val head = SyntaxDirectedHead("ImportClause", 3, 0, Rhs(List(NonTerminal("ImportedDefaultBinding", List(""), false), Terminal(","), NonTerminal("NameSpaceImport", List(""), false)), None), "BoundNames", List())
  val ids = List(
