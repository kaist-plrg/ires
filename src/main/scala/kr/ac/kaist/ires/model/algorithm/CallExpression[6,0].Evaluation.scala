...
object `AL::CallExpression[6,0].Evaluation` extends Algo {
  val head = SyntaxDirectedHead("CallExpression", 6, 0, Rhs(List(NonTerminal("CallExpression", List(""), false), NonTerminal("TemplateLiteral", List(""), false)), None), "Evaluation", List())
  val ids = List(
