...
object `AL::ForInOfStatement[3,0].ContainsUndefinedContinueTarget` extends Algo {
  val head = SyntaxDirectedHead("ForInOfStatement", 3, 0, Rhs(List(Terminal("for"), Terminal("("), NonTerminal("LeftHandSideExpression", List(""), false), Terminal("of"), NonTerminal("AssignmentExpression", List(""), false), Terminal(")"), NonTerminal("Statement", List(""), false)), None), "ContainsUndefinedContinueTarget", List(Param("iterationSet", Normal), Param("labelSet", Normal)))
  val ids = List(
