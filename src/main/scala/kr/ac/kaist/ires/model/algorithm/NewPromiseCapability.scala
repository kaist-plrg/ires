...
object `AL::NewPromiseCapability` extends Algo {
  ...
  |  3:??? "Let id:{steps} be the algorithm steps defined in link:{sec-getcapabilitiesexecutor-functions} ."
  |  4:??? "Let id:{length} be the number of non - optional parameters of the function definition in link:{sec-getcapabilitiesexecutor-functions} ."
