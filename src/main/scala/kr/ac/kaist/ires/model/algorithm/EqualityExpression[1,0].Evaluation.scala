...
object `AL::EqualityExpression[1,0].Evaluation` extends Algo {
  val head = SyntaxDirectedHead("EqualityExpression", 1, 0, Rhs(List(NonTerminal("EqualityExpression", List(""), false), Terminal("=="), NonTerminal("RelationalExpression", List(""), false)), None), "Evaluation", List())
  val ids = List(
