...
object `AL::UpdateExpression[4,0].EarlyErrors` extends Algo {
  val head = SyntaxDirectedHead("UpdateExpression", 4, 0, Rhs(List(Terminal("--"), NonTerminal("UnaryExpression", List(""), false)), None), "EarlyErrors", List())
  val ids = List(
