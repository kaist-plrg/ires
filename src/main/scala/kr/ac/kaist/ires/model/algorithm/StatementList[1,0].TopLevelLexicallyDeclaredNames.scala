...
object `AL::StatementList[1,0].TopLevelLexicallyDeclaredNames` extends Algo {
  val head = SyntaxDirectedHead("StatementList", 1, 0, Rhs(List(NonTerminal("StatementList", List(""), false), NonTerminal("StatementListItem", List(""), false)), None), "TopLevelLexicallyDeclaredNames", List())
  val ids = List(
