...
object `AL::GLOBAL.GetCapabilitiesExecutorFunctions` extends Algo {
  val head = BuiltinHead(parseRef("""GetCapabilitiesExecutorFunctions"""), List(Param("resolve", Normal), Param("reject", Normal)))
  val ids = List()
