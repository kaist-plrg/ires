...
object `AL::FormalParameters[4,0].BoundNames` extends Algo {
  val head = SyntaxDirectedHead("FormalParameters", 4, 0, Rhs(List(NonTerminal("FormalParameterList", List(""), false), Terminal(","), NonTerminal("FunctionRestParameter", List(""), false)), None), "BoundNames", List())
  val ids = List(
