...
object `AL::BindingElementList[1,0].BoundNames` extends Algo {
  val head = SyntaxDirectedHead("BindingElementList", 1, 0, Rhs(List(NonTerminal("BindingElementList", List(""), false), Terminal(","), NonTerminal("BindingElisionElement", List(""), false)), None), "BoundNames", List())
  val ids = List(
