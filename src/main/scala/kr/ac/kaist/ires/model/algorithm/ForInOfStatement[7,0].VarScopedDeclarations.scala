...
object `AL::ForInOfStatement[7,0].VarScopedDeclarations` extends Algo {
  val head = SyntaxDirectedHead("ForInOfStatement", 7, 0, Rhs(List(Terminal("for"), Terminal("await"), Terminal("("), Terminal("var"), NonTerminal("ForBinding", List(""), false), Terminal("of"), NonTerminal("AssignmentExpression", List(""), false), Terminal(")"), NonTerminal("Statement", List(""), false)), None), "VarScopedDeclarations", List())
  val ids = List(
