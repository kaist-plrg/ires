...
object `AL::StatementList[1,0].TopLevelVarDeclaredNames` extends Algo {
  val head = SyntaxDirectedHead("StatementList", 1, 0, Rhs(List(NonTerminal("StatementList", List(""), false), NonTerminal("StatementListItem", List(""), false)), None), "TopLevelVarDeclaredNames", List())
  val ids = List(
