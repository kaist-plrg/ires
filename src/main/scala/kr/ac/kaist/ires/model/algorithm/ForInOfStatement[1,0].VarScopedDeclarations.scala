...
object `AL::ForInOfStatement[1,0].VarScopedDeclarations` extends Algo {
  val head = SyntaxDirectedHead("ForInOfStatement", 1, 0, Rhs(List(Terminal("for"), Terminal("("), Terminal("var"), NonTerminal("ForBinding", List(""), false), Terminal("in"), NonTerminal("Expression", List(""), false), Terminal(")"), NonTerminal("Statement", List(""), false)), None), "VarScopedDeclarations", List())
  val ids = List(
