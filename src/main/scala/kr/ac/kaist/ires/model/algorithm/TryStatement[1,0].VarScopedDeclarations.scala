...
object `AL::TryStatement[1,0].VarScopedDeclarations` extends Algo {
  val head = SyntaxDirectedHead("TryStatement", 1, 0, Rhs(List(Terminal("try"), NonTerminal("Block", List(""), false), NonTerminal("Finally", List(""), false)), None), "VarScopedDeclarations", List())
  val ids = List(
