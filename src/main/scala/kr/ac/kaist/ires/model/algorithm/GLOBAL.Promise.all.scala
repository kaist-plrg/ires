...
object `AL::GLOBAL.Promise.all` extends Algo {
  val head = BuiltinHead(parseRef("""Promise.all"""), List(Param("iterable", Normal)))
  val ids = List(
    "sec-promise.all",
  ...
  |  } else 10:{}
  |  5:iteratorRecord
  |  6:app __x5__ = (PerformPromiseAll iteratorRecord C promiseCapability promiseResolve)
  |  6:let result = __x5__
