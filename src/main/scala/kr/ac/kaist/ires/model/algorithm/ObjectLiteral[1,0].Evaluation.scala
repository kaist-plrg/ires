...
object `AL::ObjectLiteral[1,0].Evaluation` extends Algo {
  val head = SyntaxDirectedHead("ObjectLiteral", 1, 0, Rhs(List(Terminal("{"), NonTerminal("PropertyDefinitionList", List(""), false), Terminal("}")), None), "Evaluation", List())
  val ids = List(
