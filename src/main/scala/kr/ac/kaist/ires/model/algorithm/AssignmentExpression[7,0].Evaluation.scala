...
object `AL::AssignmentExpression[7,0].Evaluation` extends Algo {
  val head = SyntaxDirectedHead("AssignmentExpression", 7, 0, Rhs(List(NonTerminal("LeftHandSideExpression", List(""), false), Terminal("||="), NonTerminal("AssignmentExpression", List(""), false)), None), "Evaluation", List())
  val ids = List(
