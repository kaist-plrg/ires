...
object `AL::Expression[0,0].HasCallInTailPosition` extends Algo {
  val head = SyntaxDirectedHead("Expression", 0, 0, Rhs(List(NonTerminal("AssignmentExpression", List(""), false)), None), "HasCallInTailPosition", List(Param("call", Normal)))
  val ids = List(
