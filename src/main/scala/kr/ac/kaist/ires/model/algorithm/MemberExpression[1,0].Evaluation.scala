...
object `AL::MemberExpression[1,0].Evaluation` extends Algo {
  val head = SyntaxDirectedHead("MemberExpression", 1, 0, Rhs(List(NonTerminal("MemberExpression", List(""), false), Terminal("["), NonTerminal("Expression", List(""), false), Terminal("]")), None), "Evaluation", List())
  val ids = List(
