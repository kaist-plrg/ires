...
object `AL::GLOBAL.Math.sin` extends Algo {
  val head = BuiltinHead(parseRef("""Math.sin"""), List(Param("x", Normal)))
  val ids = List(
    "sec-math.sin",
  ...
  |  3:??? "Return an implementation - approximated value representing the result of the sine of ℝ ( id:{n} ) ."
  |}""".stripMargin)
