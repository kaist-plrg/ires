...
object `AL::AsyncFunctionExpression[0,1].EarlyErrors` extends Algo {
  val head = SyntaxDirectedHead("AsyncFunctionExpression", 0, 1, Rhs(List(Terminal("async"), Terminal("function"), NonTerminal("BindingIdentifier", List(""), true), Terminal("("), NonTerminal("FormalParameters", List(""), false), Terminal(")"), Terminal("{"), NonTerminal("AsyncFunctionBody", List(""), false), Terminal("}")), None), "EarlyErrors", List())
  val ids = List(
