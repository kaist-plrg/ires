...
object `AL::GLOBAL.Reflect.deleteProperty` extends Algo {
  val head = BuiltinHead(parseRef("""Reflect.deleteProperty"""), List(Param("target", Normal), Param("propertyKey", Normal)))
  val ids = List(
