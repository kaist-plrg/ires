...
object `AL::ObjectBindingPattern[2,0].BindingInitialization` extends Algo {
  val head = SyntaxDirectedHead("ObjectBindingPattern", 2, 0, Rhs(List(Terminal("{"), NonTerminal("BindingPropertyList", List(""), false), Terminal("}")), None), "BindingInitialization", List(Param("value", Normal), Param("environment", Normal)))
  val ids = List(
