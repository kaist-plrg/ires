...
object `AL::ImportClause[4,0].ImportEntriesForModule` extends Algo {
  val head = SyntaxDirectedHead("ImportClause", 4, 0, Rhs(List(NonTerminal("ImportedDefaultBinding", List(""), false), Terminal(","), NonTerminal("NamedImports", List(""), false)), None), "ImportEntriesForModule", List(Param("module", Normal)))
  val ids = List(
