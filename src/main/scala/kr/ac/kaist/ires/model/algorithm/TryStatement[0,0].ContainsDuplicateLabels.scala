...
object `AL::TryStatement[0,0].ContainsDuplicateLabels` extends Algo {
  val head = SyntaxDirectedHead("TryStatement", 0, 0, Rhs(List(Terminal("try"), NonTerminal("Block", List(""), false), NonTerminal("Catch", List(""), false)), None), "ContainsDuplicateLabels", List(Param("labelSet", Normal)))
  val ids = List(
