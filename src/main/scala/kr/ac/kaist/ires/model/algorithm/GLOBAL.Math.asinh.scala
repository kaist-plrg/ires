...
object `AL::GLOBAL.Math.asinh` extends Algo {
  val head = BuiltinHead(parseRef("""Math.asinh"""), List(Param("x", Normal)))
  val ids = List(
    "sec-math.asinh",
  ...
  |  2:??? "Return an implementation - approximated value representing the result of the inverse hyperbolic sine of ℝ ( id:{n} ) ."
  |}""".stripMargin)
