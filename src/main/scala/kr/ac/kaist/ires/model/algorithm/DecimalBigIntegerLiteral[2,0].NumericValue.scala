...
object `AL::DecimalBigIntegerLiteral[2,0].NumericValue` extends Algo {
  val head = SyntaxDirectedHead("DecimalBigIntegerLiteral", 2, 0, Rhs(List(NonTerminal("NonZeroDigit", List(""), false), NonTerminal("NumericLiteralSeparator", List(""), false), NonTerminal("DecimalDigits", List(""), false), NonTerminal("BigIntLiteralSuffix", List(""), false)), None), "NumericValue", List())
  val ids = List(
    "sec-numericvalue",
