...
object `AL::ArrowParameters[0,0].IteratorBindingInitialization` extends Algo {
  val head = SyntaxDirectedHead("ArrowParameters", 0, 0, Rhs(List(NonTerminal("BindingIdentifier", List(""), false)), None), "IteratorBindingInitialization", List(Param("iteratorRecord", Normal), Param("environment", Normal)))
  val ids = List(
