...
object `AL::AsyncFunctionBody[0,0].EvaluateAsyncFunctionBody` extends Algo {
  val head = SyntaxDirectedHead("AsyncFunctionBody", 0, 0, Rhs(List(NonTerminal("FunctionBody", List(""), false)), None), "EvaluateAsyncFunctionBody", List(Param("functionObject", Normal), Param("argumentsList", Normal)))
  val ids = List(
  ...
  |  4:if (! __x2__) {
  |    3:app __x3__ = (AsyncFunctionStart promiseCapability FunctionBody)
  |    3:[! __x3__]
  |  } else {
