...
object `AL::CaseClause[0,1].ContainsDuplicateLabels` extends Algo {
  val head = SyntaxDirectedHead("CaseClause", 0, 1, Rhs(List(Terminal("case"), NonTerminal("Expression", List(""), false), Terminal(":"), NonTerminal("StatementList", List(""), true)), None), "ContainsDuplicateLabels", List(Param("labelSet", Normal)))
  val ids = List(
