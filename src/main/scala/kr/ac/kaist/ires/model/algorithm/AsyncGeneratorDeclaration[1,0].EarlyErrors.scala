...
object `AL::AsyncGeneratorDeclaration[1,0].EarlyErrors` extends Algo {
  val head = SyntaxDirectedHead("AsyncGeneratorDeclaration", 1, 0, Rhs(List(Terminal("async"), Terminal("function"), Terminal("*"), Terminal("("), NonTerminal("FormalParameters", List(""), false), Terminal(")"), Terminal("{"), NonTerminal("AsyncGeneratorBody", List(""), false), Terminal("}")), None), "EarlyErrors", List())
  val ids = List(
