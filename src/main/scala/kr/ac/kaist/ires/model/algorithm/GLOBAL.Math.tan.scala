...
object `AL::GLOBAL.Math.tan` extends Algo {
  val head = BuiltinHead(parseRef("""Math.tan"""), List(Param("x", Normal)))
  val ids = List(
    "sec-math.tan",
  ...
  |  3:??? "Return an implementation - approximated value representing the result of the tangent of ℝ ( id:{n} ) ."
  |}""".stripMargin)
