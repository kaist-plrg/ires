...
object `AL::TryStatement[1,0].ContainsUndefinedContinueTarget` extends Algo {
  val head = SyntaxDirectedHead("TryStatement", 1, 0, Rhs(List(Terminal("try"), NonTerminal("Block", List(""), false), NonTerminal("Finally", List(""), false)), None), "ContainsUndefinedContinueTarget", List(Param("iterationSet", Normal), Param("labelSet", Normal)))
  val ids = List(
