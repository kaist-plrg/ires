...
object `AL::ArrayAssignmentPattern[0,2].DestructuringAssignmentEvaluation` extends Algo {
  val head = SyntaxDirectedHead("ArrayAssignmentPattern", 0, 2, Rhs(List(Terminal("["), NonTerminal("Elision", List(""), false), Terminal("]")), None), "DestructuringAssignmentEvaluation", List(Param("value", Normal)))
  val ids = List(
