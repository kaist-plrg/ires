...
object `AL::CoverParenthesizedExpressionAndArrowParameterList[2,0].CoveredFormalsList` extends Algo {
  val head = SyntaxDirectedHead("CoverParenthesizedExpressionAndArrowParameterList", 2, 0, Rhs(List(Terminal("("), Terminal(")")), None), "CoveredFormalsList", List())
  val ids = List(
